...
public class SocketInGameEvent : ISocketEvent
{
    ...
    public void SetTcpLink(string ip, int port)
    {
        socketip = ip;
        socketPort = port;
        ISocketEngineSink();
    ...
    public bool OnEventTCPSocketRead(int main, int sub, byte[] tmpBuf, int size)
    {
        ...
        switch (main)
        {
            ...
            {
                return OnSocketMainLogon(sub, tmpBuf, size);
            } //配置信息
            case (int) (GameServer.MDM_GR_CONFIG): //2
            {
                return OnSocketMainConfig(sub, tmpBuf, size);
            } //用户信息
            case (int) (GameServer.MDM_GR_USER): //3
            {
                return OnSocketMainUser(sub, tmpBuf, size);
            } //状态信息
            case (int) (GameServer.MDM_GR_STATUS): //4
            {
                return OnSocketMainStatus(sub, tmpBuf, size);
            } //系统消息
            case (int) (GameServer.MDM_CM_SYSTEM): //1000
            {
                return OnSocketMainSystem(sub, tmpBuf, size);
            } //游戏消息
            ...
            {
                return OnSocketMainGameFrame(sub, tmpBuf, size);
            } //比赛消息
            case (int) (GameServer.MDM_GR_MATCH): //9
            {
                return OnSocketMainMatch(sub, tmpBuf, size);
            } //四人场消息
            case (int) (GameServer.MDM_GR_PRIVATE): //10
            {
                return OnSocketMainPrivate(sub, tmpBuf, size);
            }
        }
    ...
    private bool OnSocketMainLogon(int sub, byte[] data, int dataSize)
    {
        ...
        {
            //登录成功
            case (int) (MDM_GR_LOGON.SUB_GR_LOGON_SUCCESS): //1---100
            {
                return OnSocketSubLogonSuccess(data, dataSize);
            } //登录失败
            case (int) (MDM_GR_LOGON.SUB_GR_LOGON_FAILURE): //1---101
            {
                return OnSocketSubLogonFailure(data, dataSize);
            } //登录完成
            case (int) (MDM_GR_LOGON.SUB_GR_LOGON_FINISH): //1---102
            {
                return OnSocketSubLogonFinish(data, dataSize);
            } //更新提示
            case (int) (MDM_GR_LOGON.SUB_GR_UPDATE_NOTIFY): //1---200
            {
                return OnSocketSubUpdateNotify(data, dataSize);
            } //响应录像列表
            case (int) (MDM_GR_LOGON.SUB_RESPONSE_REPLAY_LIST): //1---412
            {
                return OnSocketSubReplayList(data, dataSize);
            } //响应录像数据
            case (int) (MDM_GR_LOGON.SUB_RESPONSE_REPLAY_DATA): //1---413
            {
    ...
    private bool OnSocketSubLogonFailure(byte[] data, int dataSize)
    {
        ...
        MyDebug.Log("IErrorCode:" + pGameServer.lErrorCode + "--" + NetUtil.GetServerLog(pGameServer.szDescribeString));
        SocketEventHandle.Instance.iscloseLoading = true;
