...
public class BundleUpdate : MonoSingleton<BundleUpdate>
{
    ...
    {
        UIPanel_SceneLoading.instance.tips.text = "正在计算更新文件";
        foreach (KeyValuePair<string, List<string>> pair in DicMD5)
        {
    ...
    {
        WWW www = new WWW(url);
        while (!www.isDone && DicMD5.Count > 0)
        {
            currentNum = (long)(www.progress * int.Parse(DicMD5[DicMD5UpdateName[updateCount]][1]));
